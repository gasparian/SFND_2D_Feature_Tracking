...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &mask, cv::Mat &descriptors, string descriptorType)
{
    ...
    else if (descriptorType.compare("FREAK") == 0)
    {
        ...
        int octaves = 4;
        extractor = cv::xfeatures2d::FREAK::create(true, true,  patternScale, octaves, vector< int >());
    }
...
void harris_nms(cv::Mat& dst_norm, vector<cv::KeyPoint>& keypoints, int& apertureSize, int& minResponse, double maxOverlap) {
    for (size_t j = 0; j < dst_norm.rows; j++)
    {
        for (size_t i = 0; i < dst_norm.cols; i++)
        {
            ...
            { // only store points above a threshold
                ...
                cv::KeyPoint newKeyPoint;
                newKeyPoint.pt = cv::Point2f(i, j); // convert (rows, cols) --> (x, y)
                newKeyPoint.size = apertureSize;
...
void detKeypointsHarris(vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    harris_nms(dst_norm, keypoints, blockSize, minResponse, maxOverlap);
